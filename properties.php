...
$images = "SELECT count(property_id) FROM property group by property_id ";
...
$result = mysqli_query($conn, "SELECT dimensions,link,location,status,map,p.property_id FROM photos ph,property p where p.property_id=ph.property_id  group by p.property_id order by reg_date DESC LIMIT $offset, $per_page");
